...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated == True:
           ...
           canvas.draw_image(explosion_image, current_rock_center, ROCK_SIZE, self.pos, ROCK_SIZE) 
           time += 0.5
    ...
    def collide(self,other_object):
        global info
        if (other_object.radius+self.radius)>math.sqrt((other_object.pos[0]-self.pos[0])**2+(other_object.pos[1]-self.pos[1])**2):
           return True
        else: 
           return False
...
def group_collide(group,my_ship):
    global lives, explosion_group,rock_group
    destroy=False
    d_group=set()
    for i in group:
        if i.collide(my_ship)==True:
           d_group.add(i)
           explosion_group.add(i)
           for j in explosion_group:
                j.animated = True
                j.lifespan=30
                j.age=0
                explosion_sound.play()
           destroy=True
    group.difference_update(d_group)   
    return destroy
...
def group_group_collide(missile_group,rock_group):
    ...
    for i in rock_group:
        if group_collide(missile_group,i):
            d_group.add(i)
            score +=1
    rock_group.difference_update(d_group)        
...
def draw(canvas):
    ...
    if lives == 0:
        ...
        explosion_group.intersection_update(set([]))
